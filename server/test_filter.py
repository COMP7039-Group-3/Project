...
import unittest
from filter import filter_by_category
...
class TestFilter(unittest.TestCase):
    ...
    def test_all_results_belong_to_selected_category(self):
        """
        TC01	Verify that if a category is selected, all results obtained belong to that category
        """
        selected_category = "news/business"
        res = filter_by_category(self.mock_article_list, selected_category)

        for item in res:
            self.assertEqual(item["section"], selected_category)

    def test_no_category_is_selected(self):
        """
        TC03	Verify that if no category is selected, the collection is not filtered
        """
        selected_category = ""
        res = filter_by_category(self.mock_article_list, selected_category)
    ...
    def test_No_error_throw_if_category_does_not_exists(self):
        """
        TC04	Verify that if there’s no item in the collection for a specific category no error is returned.
        """
        selected_category = "fakecat"
        res = filter_by_category(self.mock_article_list, selected_category)
    ...
    def test_category_does_not_exists_should_return_empty_collection(self):
        """
        TC05	Verify that if there are no items for any of the categories selected, an empty collection is returned.
        """
        selected_category = "fakecat"
        res = filter_by_category(self.mock_article_list, selected_category)
